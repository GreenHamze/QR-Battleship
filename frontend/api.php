<?php
/**
 * api.php — PHP proxy to Node.js backend using file_get_contents.
 * Required because the Lehre server firewall blocks direct access to Node.js ports.
 * Forwards all requests from Apache (port 80/443) to localhost:3000.
 */
...
if ($response === false) {
    ...
    echo json_encode(['error' => 'backend_unreachable', 'hint' => 'Is the Node.js backend running? Try: cd ~/battleship-server && node app.js']);
    exit;
}
